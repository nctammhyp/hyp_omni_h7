...
class UpdateBlock(nn.Module):
    ...
    def forward(self, net, inp, corr=None, inv_depth=None, no_upsample=False):
        ...
        # === safe mask computation ===
        if net.numel() == 0:
            # fallback nếu net rỗng
            mask = torch.zeros(1, (2**self.opts.num_downsample)**2 * 9, 1, 1, device=net.device)
        else:
            mask = .25 * self.mask(net)
            # đảm bảo không có NaN/Inf
            mask = torch.nan_to_num(mask, nan=0.0, posinf=1e6, neginf=-1e6)
