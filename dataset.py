...
class Dataset(torch.utils.data.Dataset):
    ...
    def loadGTInvdepthIndex(self, fidx, remove_gt_noise=True,
            ...
            # --- FIX STARTS HERE ---
            # Ensure gt is 2D. If it's (H, W, C), take the first channel.
            if gt.ndim == 3:
                gt = gt[:, :, 0]
            # -----------------------

            gt_h = gt.shape[0]
            # (Optional) If you need to resize/crop to match equirect_size h, w:
            ...
            # Ensure finite_depth is 2D (H, W)
            finite_depth = gt >= 1e-3 
            
            # This will now work because finite_depth is 2D and morph_filter is 2D
            closed_depth = scipy.ndimage.binary_closing(
                finite_depth, structure=morph_filter) 
