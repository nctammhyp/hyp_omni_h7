...
class Dataset(torch.utils.data.Dataset):
    ...
    def __loadOrBuildLookupTable(self) -> None:
        ...
        if not osp.exists(path):
            ...
            grids = np.fromfile(path, dtype=np.float32).reshape(
                [self.num_cam, h, w, int(self.num_invdepth / 2**self.num_downsample), 2])
            self.grids = [grids[i, ...].squeeze() for i in range(self.num_cam)]
    # end __initSweep
    ...
    def buildLookupTable(self, transform=None, phi_deg=None, phi2_deg=None, output_gpu_tensor=False) -> list:
        ...
        if output_gpu_tensor:
            grids = [torch.zeros((h, w, num_invdepth, 2),
                                 requires_grad=False).cuda() for _ in range(self.num_cam)]
        else:
            grids = [np.zeros((h, w, num_invdepth, 2), dtype=np.float32) for _ in range(self.num_cam)]
    ...
    def loadGTInvdepthIndex(self, fidx, remove_gt_noise=True,
        ...
        # --- FIX 2: Resize GT về đúng kích thước train (160x640) ---
        # Nếu kích thước khác biệt, resize ngay lập tức
        if gt.shape[0] != h or gt.shape[1] != w:
            # Dùng INTER_NEAREST để giữ giá trị depth gốc chính xác hơn, tránh nội suy làm mờ biên
            gt = cv2.resize(gt, (w, h), interpolation=cv2.INTER_NEAREST)
        # -----------------------------------------------------------

        gt_h = gt.shape[0]
        # (Đoạn code crop cũ dưới này sẽ không chạy nữa vì kích thước đã khớp, nhưng cứ để đó)
        if h < gt_h:
    ...
    def makeVisImage(self, imgs, invdepth: np.ndarray, gt=None, transform=None, return_all=False):
        ...
        inputs = concat([imgs[0], imgs[1], imgs[2]], axis=1)
...
class Augmentor:
    ...
    def color_transform(self, raw_images):
            ...
            # symmetric
                ...
                # --- CŨ (Gây lỗi chiều cao 576) ---
                # images = list(np.split(image_stack, 4, axis=0))
                
                # --- MỚI (Tự động chia theo số lượng ảnh đầu vào - là 3) ---
                images = list(np.split(image_stack, len(raw_images), axis=0))
    ...
    def eraser_transform(self, images, bounds=[50, 100]):
            ...
            ht, wd = images[0].shape[:2]
            if np.random.rand() < self.eraser_aug_prob:
                # --- CŨ (Gây lỗi vì không có images[3]) ---
                # for i in [1, 3]: 
                
                # --- MỚI (Sửa thành index các camera cần augment, ví dụ 1 và 2) ---
                for i in [1, 2]: 
                    mean_color = np.mean(images[i].reshape(-1, 3), axis=0)
                    for _ in range(np.random.randint(1, 3)):
                        x0 = np.random.randint(0, wd)
